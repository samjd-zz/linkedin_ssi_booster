...
1. **ハイブリッドRAG & 応答検証**: BM25によるキーワードスコアリング、Model2Vecの意味類似度、NetworkXの構造解析、spaCyの意味検証を融合した多段階のTruth Gate。
2. **ペルソナ駆動の継続学習**: persona graph、ドメイン知識、RSSから抽出した知識、ナラティブメモリを統合し、根拠付きの会話とコンテンツ生成に再利用します。
3. **SSI向けコンテンツ・オーケストレーション**: Buffer連携、4つのSSI指標、信頼度ポリシー、候補選択学習を組み合わせ、生成・レビュー・公開の流れを自動化します。
4. **ローカル・マルチモーダル実行**: Ollamaを中心に、FLUX Capacitor、Piper TTS、Rei ToeiのSuno/Strudel音楽ノードをDockerプロファイルで分離運用します。

### 🇯🇵 日本語のNLPと継続学習 (Japanese NLP & Continual Learning)

本システムは**日本語の記事を翻訳経由ではなく原文から直接学習**します。日本語RSSは固定リストに埋め込まれておらず、`CURATOR_RSS_FEEDS` で置き換えるか、`CURATOR_RSS_FEEDS_EXTRA` で追加できます。設定された日本語ソースは、SudachiPyと`ja_core_news_md`を使う同じ継続学習パイプラインに入り、英語記事と共通の知識グラフ・BM25検索・根拠検証へ統合されます。

- 🧭 **自動言語ルーティング** — ひらがな・カタカナ・漢字を検出し、`ja_core_news_md` と `en_core_web_md` を自動で切り替え
- 🧩 **形態素解析と検索** — SudachiPyによるトークナイズ、NER、名詞句抽出、意味類似度、lemmaベースのBM25検索を一つの流れで処理
- 🔎 **抽出・要約・重複排除** — 日本語の事実を正規化して知識グラフへ統合し、生成時の根拠として再利用
- 🔤 **決定論的な発音支援** — `pykakasi`で漢字・かなをHepburn式ローマ字へ変換し、Rei Toeiの歌詞キューとSamの学習用発音ガイドを安定化
- 🔮 **ドメイン知識に基づく漢字学習** — Samは一般的なLLM知識だけに頼らず、読み方・意味・用例・出典を読み込んだ日本語パックから説明を組み立てます
- 🪄 **視覚学習への接続** — `draw`、`render`、`/art [topic]`を使うと、根拠付きの漢字レッスンをFLUX Capacitorへ渡し、書道・字形・文化語彙の視覚教材として生成できます
- 📣 **日本語の告知表現** — 「発表」「開発」「公開」「導入」などの表現を記事要約のシグナルとして認識

### 📚 日本語学習カリキュラム (Japanese Learning Curriculum)

Samの日本語知識は、独立した複数の`domain_knowledge_*.json`パックを一つの証拠プールへ自動統合します。基本の学習順序は、**ローマ字 → ひらがな → カタカナ → 漢字の構造と読み → 実用漢字200**です。そこに文法、助詞、活用、丁寧さ、語彙形成、発音・モーラ、ピッチアクセント、学習者レベル、想起練習と間隔反復の知識を重ねます。

- 📝 **文字から文脈へ** — ローマ字を入力・読解の橋として使い、かな、漢字、語彙、例文へ段階的に進む
- 🎧 **音とリズム** — モーラ、長音、促音、母音無声化、ピッチアクセントを歌詞と会話の両方で扱う
- 🧠 **学習者中心の説明** — 認識・読解・産出を分け、想起練習、間隔反復、誤りの訂正を組み合わせる
- 🗂️ **根拠の境界** — 個別の漢字の意味・読み・新しい例文は辞書などの最新資料で確認すべきものとして扱い、永続的な学習原則と混同しません
...
- 🧠 **Advanced multi-language NLP with spaCy** — theme/claim extraction, semantic similarity, fact suggestion when the truth gate drops a sentence, and multi-language routing (English `en_core_web_md` + Japanese `ja_core_news_md` with the SudachiPy tokenizer). The curator can ingest configured Japanese-language sources alongside the general feed set, so the Japanese pipeline can learn from real native-language articles rather than translated text. Preprocessing filters boilerplate before fact storage. See [docs/spacy-extraction.md](docs/spacy-extraction.md) and [docs/knowledge-extraction-improvement.md](docs/knowledge-extraction-improvement.md).
...
The curator supports Japanese-language RSS sources alongside its general AI, search, Java, cloud, and GovTech feeds. Japanese feeds can be supplied through `CURATOR_RSS_FEEDS` or appended with `CURATOR_RSS_FEEDS_EXTRA`; the current default configuration does not hard-code a fixed Japanese feed list. When native Japanese sources are configured, the `ja_core_news_md` pipeline is exercised end to end: SudachiPy tokenization, NER, semantic similarity, lemma-based BM25 retrieval, and fact extraction into the same continual-learning knowledge graph used by English articles.

Routing is automatic — `detect_language()` checks for Hiragana, Katakana, or Kanji and picks the matching model from `SPACY_MODELS`. Keyword filtering is substring-based, so Japanese feeds need Japanese keywords; `CURATOR_KEYWORDS_EXTRA` appends them without discarding the active defaults. `CURATOR_RSS_FEEDS_EXTRA` likewise appends feeds without replacing the configured list.

**Known limitations, stated plainly:** `trafilatura` is the primary article extractor and normally removes navigation, sidebars, and cookie banners. The fallback cleaner still relies on English-oriented HTML, entity, bracket, whitespace, and WordPress-footer rules, so Japanese site chrome or boilerplate can survive when extraction falls back or when source text is noisy. Japanese extraction is therefore more dependent on source quality and can be noisier per fact than English, even though it remains useful as retrieval evidence.
