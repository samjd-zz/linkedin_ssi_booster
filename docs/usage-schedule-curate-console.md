...
When asking Sam for artwork or character visuals in console mode, the system detects image intent using the following trigger sets. Any **Image Request Verb** alone is sufficient to auto-trigger a FLUX render; a **Japanese Character & Art Subject Marker** is not required to trigger the render, but when one is present alongside a verb, the request is additionally flagged as a Japanese-art request for downstream handling.
...
- _"Draw the hiragana character あ with stroke order details"_
- _"Render kanji 夢 in golden calligraphy on dark silk"_
- _"Paint an illustration of a samurai warrior in ukiyo-e style"_
- _"Generate an image of the katakana letter カ"_
